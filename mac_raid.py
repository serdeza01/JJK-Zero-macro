...
try:
    ...
    print("CRITICAL ERROR: 'pynput' library is missing.")
    exit()

pyautogui.FAILSAFE = False

class JujutsuMacroMac:
    def __init__(self, root):
        self.root = root
        self.root.title("Jujutsu Zero Macro (Mac - by serdeza)")
        ...
        self.keys = {"record": "p", "replay": "m", "stop": "Key.f2"}
        self.load_settings()
    ...
    def setup_ui(self):
        ...
        ttk.Label(self.root, text="JUJUTSU ZERO MACRO", style="Bold.TLabel").pack(pady=10)
    ...
    def setup_manager_tab(self, parent):
        ...
        save_group = ttk.LabelFrame(parent, text="Save Recording")
        ...
        self.ent_filename.insert(0, "Mac_Run")
        ...
        ttk.Label(grp_speed, text="Multiplier (1.0 = Normal):").pack(side="left", padx=5)
    ...
    def export_macro(self):
        ...
        dest_path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON Files", "*.json")], initialfile=fname, title="Export Macro")
        if dest_path:
            shutil.copy(src_path, dest_path)
            messagebox.showinfo("Success", f"Exported to {dest_path}")

    def import_macro(self):
        src_path = filedialog.askopenfilename(filetypes=[("JSON Files", "*.json")], title="Import Macro")
        if src_path:
    ...
    def refresh_file_list(self):
        ...
        for f in files: self.macro_listbox.insert(tk.END, os.path.basename(f))
    def on_macro_select(self, event):
        selection = self.macro_listbox.curselection()
        if selection: self.load_macro_from_disk(self.macro_listbox.get(selection[0]))
    def load_macro_from_disk(self, filename):
        path = os.path.join(self.macro_folder, filename)
        if os.path.exists(path):
            with open(path, "r") as f: self.recorded_events = json.load(f)
    ...
    def save_current_recording(self):
        ...
        with open(path, "w") as f: json.dump(self.recorded_events, f)
        self.refresh_file_list()
    def delete_macro(self):
        selection = self.macro_listbox.curselection()
        if not selection: return
        path = os.path.join(self.macro_folder, self.macro_listbox.get(selection[0]))
    ...
    def is_key_match(self, key_obj, saved_str):
        try:
            if hasattr(key_obj, 'char') and key_obj.char: k_str = key_obj.char
            else: k_str = str(key_obj)
            return k_str == saved_str
        except: return False
    def get_key_str(self, key_obj):
        try:
            if hasattr(key_obj, 'char') and key_obj.char: return key_obj.char
            else: return str(key_obj)
        except: return "Unknown"
    def log(self, text): self.root.after(0, lambda: self.status_var.set(text))
    def set_mode(self, mode): self.active_mode.set(mode)
    ...
    def global_on_release(self, key):
        if self.recording:
            if (self.is_key_match(key, self.keys['record']) or self.is_key_match(key, self.keys['replay']) or self.is_key_match(key, self.keys['stop'])): return
            k_str = self.get_key_str(key)
            self.record_event({'type': 'release', 'key': k_str})
    def global_on_move(self, x, y):
        if self.recording: self.record_event({'type': 'move', 'x': x, 'y': y})
    def global_on_click(self, x, y, button, pressed):
        if self.recording: self.record_event({'type': 'click', 'x': x, 'y': y, 'button': str(button), 'pressed': pressed})
    def global_on_scroll(self, x, y, dx, dy):
        if self.recording: self.record_event({'type': 'scroll', 'x': x, 'y': y, 'dx': dx, 'dy': dy})
    ...
    def record_event(self, data):
        ...
        if len(self.recorded_events) % 50 == 0:
    ...
    def run_recorded_macro(self):
        ...
        while self.running:
            ...
            for event in self.recorded_events:
                ...
                etype = data['type']
                try:
                    if etype == 'move': self.mouse_controller.position = (data['x'], data['y'])
                    elif etype == 'click':
                        btn = Button[data['button'].split('.')[-1]]
                        if data['pressed']: self.mouse_controller.press(btn)
                        else: self.mouse_controller.release(btn)
                    elif etype == 'scroll': self.mouse_controller.scroll(data['dx'], data['dy'])
                    elif etype == 'press': self.keyboard_controller.press(self.parse_key(data['key']))
                    elif etype == 'release': self.keyboard_controller.release(self.parse_key(data['key']))
                except: pass
            time.sleep(1)
    ...
    def run_standard_macro(self):
        while self.running:
            self.log("Standard Mode: Looking for 'retry_mac.png'...")
            found_retry = False
            loops = 0
            while self.running and not found_retry and loops < 20:
                try:
                    btn_pos = pyautogui.locateCenterOnScreen('retry_mac.png', confidence=0.7, grayscale=True)
                    if btn_pos:
                        pyautogui.moveTo(btn_pos[0]/2, btn_pos[1]/2)
                        time.sleep(0.2)
                        pyautogui.click()
                        time.sleep(0.1)
                        pyautogui.click()
            ...
            self.log(f"Waiting load... ({int(delay)}s)")
            time.sleep(delay)
